...
def clear_image():
    st.session_state.response = st.session_state.response[0], None
    st.rerun()
...
uploaded_file = st.sidebar.file_uploader("Choose an image...", type=["png"], key="file_uploader")
...
with col1:
    ...
    if uploaded_file is not None:
        ...
        st.text("You can upload an image at the left\nsidebar to send with your question")

# Chat interaction
with col2:
    st.header("Response")
    st.text_area("Text Response", value=st.session_state.response[0], height=370)
    if st.session_state.response[1] is not None:
        st.text("Image Response")
        gen_image = Image.open(io.BytesIO(base64.b64decode(st.session_state.response[1])))
